      ...
      <div className="p-8 max-w-[1600px] mx-auto">
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => navigate('/engineering')} className="flex items-center gap-1 text-sm text-primary hover:text-primary/80 transition-colors">
              <span>Explore all modules</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {reportHighlights.slice(0, 8).map((highlight, index) => <HighlightCard key={highlight.id} highlight={highlight} index={index} onClick={() => navigate('/engineering')} />)}
          </div>
        </section>
      </div>
    </MainLayout>;
