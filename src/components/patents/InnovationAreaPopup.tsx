...
function TechnologyPopup({ tech, idx }: { tech: Technology; idx: number }) {
      ...
      <DialogContent className="max-w-lg p-0 overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 border-b bg-card">
          <h5 className="text-sm font-medium text-foreground mb-3 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground font-normal ml-1">(click for details)</span>
          </h5>
          <div className="grid grid-cols-2 gap-2">
            {area.technologies.map((tech, idx) => (
              <TechnologyPopup key={tech.name} tech={tech} idx={idx} />
            ))}
          </div>
        </div>
