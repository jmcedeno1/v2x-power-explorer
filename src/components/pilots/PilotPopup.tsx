        ...
        <ScrollArea className="max-h-[90vh]">
          <div className="p-6 space-y-5">
            <DialogHeader>
              <div className="flex items-center gap-3 mb-2">
                <div className="text-3xl font-bold text-primary">
                  Case: {String(pilot.name).split(' ')[0]}
                </div>
                {pilot.maturity && (
                  <Badge variant="outline" className={cn('text-xs capitalize', maturityColors[pilot.maturity])}>
              ...
              <div className="lg:col-span-3 space-y-3">
                ...
                {media?.businessModel && (
                  <InfoSection title="Business Model" icon={<Building2 className="w-4 h-4" />} accentColor="text-primary">
                    ...
                    <p className="text-sm text-muted-foreground">{media.businessModel.description}</p>
                  </InfoSection>
                )}
                {media?.standardization && (
                  <InfoSection title="Standardization Overview" icon={<FileText className="w-4 h-4" />} accentColor="text-energy-blue">
                    <p className="text-sm text-foreground">
                      Standard: <span className="font-bold">{media.standardization.standard}</span>
                    </p>
                    <p className="text-sm text-muted-foreground">{media.standardization.status}</p>
                  </InfoSection>
                )}
              </div>
            ...
            {(media?.technology || media?.timeline) && (
              <div className="grid lg:grid-cols-2 gap-5">
                {media?.technology && (
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
                      <Zap className="w-5 h-5 text-primary" /> Technology Overview
                    </h3>
                    <div className="flex justify-center">
                      <Badge className="bg-primary text-primary-foreground px-4 py-2 text-sm font-medium">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <TechCategory title="Hardware" items={media.technology.hardware} color="bg-energy-amber" />
                      <TechCategory title="Software" items={media.technology.software} color="bg-energy-green" />
                    </div>
                  </div>
                )}
                ...
                {media?.timeline && (
                  <div className="p-5 rounded-xl bg-gradient-to-br from-primary/5 to-accent/5 border border-primary/20">
                    ...
                    {media.partnerLead && (
                      <p className="text-sm text-muted-foreground mb-4">{media.partnerLead}</p>
                    )}
                    <div className="relative">
                      <div className="absolute left-4 top-4 bottom-4 w-0.5 bg-gradient-to-b from-primary/60 to-primary/20" />
                      <div className="space-y-4">
                        {media.timeline.map((item, index) => (
